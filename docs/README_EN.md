...
This edition has been **simplified and pedagogically adapted for youth and students aged 10 to 18 years old**, introducing a **tangible energy endowment (Energy Units / Piggy Bank)** to make the value of personal endowments and the public-goods trade-off immediately intuitive.

---

## 1. Narrative & Overview for Young Participants (10-18 Years)

You are **Marty McFly**, in 1985. The **DeLorean** is stranded and the **Flux Capacitor** is drained: to travel back to the future, you need **1.21 GW** of power. Doc Brown needs funds to fuel the DeLorean.
...
### Energy Endowment and the Personal Piggy Bank
In each of the 5 rounds, every player receives **10 personal units of energy (⚡)**:
1. **🔒 In your Piggy Bank**: units of energy you choose NOT to donate remain safely in your personal account.
2. **⚡ In the DeLorean Fund**: units of energy donated by you and the other players are summed up, multiplied by **1.6** by Doc (a 60% increase!), and divided equally among all 4 participants.
3. **💰 Round Earnings**: `Units of energy kept in piggy bank + your equal share of the common fund`.

---

## 2. Collective Team Goal (1.21 GW - 100 Energy Units)

The game blends a repeated public goods game with an intertemporal **Collective-Risk Social Dilemma (CRSD)**:

- **Round endowment**: `ENDOWMENT = 10` units of energy per player (50 total across 5 rounds).
- **Multiplier**: `MULTIPLIER = 1.6`.
- **Number of players**: `N_PLAYERS = 4` (Marty + Doc + Biff + Jennifer).
- **Number of rounds**: `NUM_ROUNDS = 5`.
- **Collective target**: `CUMULATIVE_TARGET_MONEY = 100` total units of energy donated by the group across 5 rounds (equivalent to 1.21 GW). 100 units represents a 50% overall cooperation rate (maximum capacity: `4 * 10 * 5 = 200`).

### Final Verdict (Round 5)
- If the team donates **at least 100 units of energy (1.21 GW)**:
  **"88 MPH - Time travel successful!"** The DeLorean hits 88 MPH.
- If the team donates **fewer than 100 units of energy**:
  **"Time Paradox!"** The car runs dry and does not start.

> **No loss possible**: in both cases the participant **always** keeps all the units of energy
...
Always contributes **10 units of energy every round** to build a solid baseline for the mission (50 units total).

### Biff Tannen - *Selfish / Pure Free Rider*
Always contributes **0 units of energy**, keeping everything in his piggy bank and taking advantage of others.

### Jennifer Parker - *Reciprocal / Conditional Cooperator*
- **Round 1**: contributes a friendly start of **5 units of energy**.
...
- **Bot** (`marty_strategy = 'tit_for_tat'`): contributes 5 units of energy on round 1, then the rounded average of the other 3 players.
...
1. **Dual Real-Time Counter (`DecisionPage.html`)**:
   ...
   - 🟢 **In your Piggy Bank**: `10 - X` units of energy (bright green)
   - ⚡ **In the DeLorean Fund**: `X` units of energy (bright orange)
...
2. **Step-by-Step Earnings Calculation (`ResultsPage.html`)**:
   ...
   `Units of energy kept in piggy bank + share of the common fund = round earnings`, alongside the running total in the piggy bank.

3. **Battery Progress Bar**:
   Real-time progress towards 1.21 GW and the required 100 units of energy.
