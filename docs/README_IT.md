...
Questa versione è stata specificamente **semplificata e ottimizzata didatticamente per ragazzi e studenti dai 10 ai 18 anni**, introducendo una **dotazione energetica tangibile (unità di energia / Salvadanaio)** per rendere immediatamente percepibile l'importanza delle proprie risorse e il trade-off con il bene pubblico.

---

## 1. Descrizione generale e narrativa per ragazzi (10-18 anni)

Sei **Marty McFly**, nel 1985. La **DeLorean** è ferma e il **Flusso Canalizzatore** (*Flux Capacitor*) è scarico: per viaggiare nel tempo e tornare al futuro serve una potenza di almeno **1.21 GW**. Doc Brown ha bisogno di fondi per alimentare la DeLorean.
...
### La dotazione energetica e il Salvadanaio personale
In ciascuno dei 5 round ogni giocatore riceve **10 unità di energia personali (⚡)**:
1. **🔒 Nel tuo Salvadanaio**: le unità di energia che decidi di NON donare restano subito al sicuro nel tuo salvadanaio personale.
2. **⚡ Nel fondo DeLorean**: le unità di energia donate da te e dagli altri vengono sommate, moltiplicate per **1.6** da Doc (crescono del 60%!) e divise in 4 parti uguali tra tutti i partecipanti.
3. **💰 Guadagno del round**: `Unità di energia tenute nel salvadanaio + quota del fondo comune`.

---

## 2. Obiettivo finale collettivo (1.21 GW - 100 unità di energia)

Il gioco combina la dinamica dei beni pubblici ripetuta con un **Collective-Risk Social Dilemma (CRSD)** intertemporale:

- **Dotazione per round**: `ENDOWMENT = 10` unità di energia a giocatore (50 totali in 5 round).
- **Fattore moltiplicativo**: `MULTIPLIER = 1.6`.
- **Numero di giocatori**: `N_PLAYERS = 4` (Marty + Doc + Biff + Jennifer).
- **Numero di round**: `NUM_ROUNDS = 5`.
- **Soglia di successo collettivo**: `CUMULATIVE_TARGET_MONEY = 100` unità di energia complessive donate dal gruppo in 5 round (pari a 1.21 GW). 100 unità corrisponde al 50% di cooperazione complessiva del gruppo (capacità massima: `4 * 10 * 5 = 200`).

### Verdetto finale (Round 5)
- Se a fine gioco il gruppo ha donato **almeno 100 unità di energia (1.21 GW)**:
  **"88 MPH - Viaggio nel tempo riuscito!"** La DeLorean parte e raggiunge le 88 MPH.
- Se il gruppo ha donato **meno di 100 unità di energia**:
...
> tutte le unità di energia accumulate nel proprio salvadanaio (`final_game_payoff` = totale
...
Dona **sempre 10 unità di energia** a ogni round per garantire una solida base per la macchina del tempo (50 unità complessive).

### Biff Tannen - *Egoista / Pure Free Rider*
Dona **sempre 0 unità di energia**, tenendo tutto nel proprio salvadanaio e approfittando della generosità altrui.

### Jennifer Parker - *Reciproca / Conditional Cooperator*
- **Round 1**: offre un contributo amichevole di **5 unità di energia**.
...
- **Umano** (`marty_strategy = 'human'`): gioca tramite la dashboard interattiva con visual feedback immediato in tempo reale (doppio contatore *Salvadanaio* vs *Fondo DeLorean*).
- **Bot automatico** (`marty_strategy = 'tit_for_tat'`): dona 5 unità di energia al round 1, poi la media arrotondata degli altri 3 giocatori.
...
1. **Doppio contatore dinamico (`DecisionPage.html`)**:
   ...
   - 🟢 **Nel tuo Salvadanaio**: `10 - X` unità di energia (in verde neon)
   - ⚡ **Nel fondo DeLorean**: `X` unità di energia (in arancione)
...
2. **Calcolo passo-passo trasparente (`ResultsPage.html`)**:
   ...
   `Unità di energia tenute + quota del fondo comune = guadagno round`, con il saldo progressivo del proprio salvadanaio.

3. **Barra di carica a batteria**:
   Progresso visivo costante verso i 1.21 GW e le 100 unità di energia necessarie.
